...
function set_tempest_conf {
    ...
    iniset $conf_file identity uri $OS_AUTH_URL
    iniset $conf_file identity uri_v3 $OS_AUTH_URL
...
(cd $TEMPEST_DIR; stestr init)
(cd $TEMPEST_DIR; stestr list monasca_tempest_tests > monasca_tempest_tests)
(cd $TEMPEST_DIR; cat monasca_tempest_tests)
(cd $TEMPEST_DIR; cat monasca_tempest_tests | grep gate > monasca_tempest_tests_gate)
(cd $TEMPEST_DIR; stestr run --subunit --load-list=monasca_tempest_tests_gate | subunit-trace --fails)
